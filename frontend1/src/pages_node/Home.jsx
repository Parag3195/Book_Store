...
useEffect(()=>{
setLoading(true);
axios.get(`${node_BASE_URL}/books`).then((response)=>{
...
setLoading(true);
axios
.post('http://localhost:5555/books',data).then(()=>{
    setLoading(false);
    console.log(data);
    // navigate('/');
    ...
    <div key={book._id} className="col-md-3">
      ...
      <h5 style={{ textAlign: 'left' }}>{index + 1} .</h5>
        ...
        <div className="d-flex justify-content-center">
          <div className="p-2">
            <Link to={`/node/books/details/${book._id}`}>
              <BsFillInfoCircleFill size={25} color="black" />
            </Link>
          </div>
          <div className="p-2">
            <Link to={`/node/books/edit/${book._id}`}>
              <FaEdit size={25} color="black" />
            </Link>
          </div>
          <div className="p-2">
            <Link to={`/node/books/delete/${book._id}`}>
